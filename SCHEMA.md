...
Every interface exports `controller_profile_match` with policy `usb-controller-match-v1`, `candidate`, `status` (`no_profile`, `matched` or `rejected`), `definition_matches`, `physical_binding_verified`, `device_authentication_verified` and `reasons`. A structural definition match alone cannot grant physical authority to an offline or logical view. `physical_binding_verified` additionally requires a resolved physical device, supported scope, configuration and alternate setting. `device_authentication_verified` is always false.

Sony matching checks its known family/variant identity, non-boot HID interface, interrupt IN/OUT pair and endpoint capacities. The Input layout must contain a single controller collection with the expected report ID, length and control anchors, including offsets, widths, usage identities, ranges and relevant flags. DS4 and DualSense cannot cross-match merely because both use 64-byte Input reports. A Sony PPD reconstruction must also have exact Input framing, complete interpretation and passing report-specific Windows validation. GIP/XUSB matching checks the physical vendor-interface signature and interrupt pair; GIP data is limited to interface 0. Third-party transport signatures remain eligible. Unknown Sony identities are not assigned known Sony semantics.
